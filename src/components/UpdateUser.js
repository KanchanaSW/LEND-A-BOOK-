...
const Update = (props) => {
  ...
  useEffect(() => {
    ...
    userService.getUserDetails(currentUser.id).then((res) => {
      let user = res.data;
  ...
  const handleUpdateUser = (e) => {
    ...
    if (checkBtn.current.context._errors.length === 0) {
      userService.updateUser(currentUser.id,password, fullname, dob, image).then(
        (response) => {
  ...
  return (
    <div className="card3 card-container-3" style={{ width: "400px" }}>
      <h3 style={{ marginLeft: "77px" }}>Update User</h3>
